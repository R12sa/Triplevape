...
run(function()
		...
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
					...
					if newchannel then
						newchannel:SendAsync('; I USING Triplevape')
					end
					textChatService.ChatInputBarConfiguration.TargetTextChannel = oldchannel
					textChatService.ChannelTabsConfiguration.Enabled = false
				elseif replicatedStorage:FindFirstChild('DefaultChatSystemChatEvents') then
					replicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer('/w '..v.Name..' ; I USING Triplevape', 'All')
				end
			end
		end
	end	
	...
	function whitelist:process(msg, plr)
		if plr == lplr and msg == '; I USING Triplevape' then return true end
		if self.localprio > 0 and not self.said[plr.Name] and msg == '; I USING Triplevape' and plr ~= lplr then
	...
	function whitelist:hook()
			...
			if exp and exp:WaitForChild('appLayout', 5) then
				vape:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
					...
					if obj and obj:IsA('TextLabel') then
						if obj.Text:find('; I USING Triplevape') then
		...
		if exp then
			...
			if bubblechat then
				vape:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
					if newbubble:IsA('TextLabel') and newbubble.Text:find('; I USING Triplevape') then
		...
		-- Commands without whitelist dependency
		local commands = {
			byfron = function()
				while wait() do
					pcall(function()
			...
			reveal = function()
				task.delay(0.1, function()
					if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
						textChatService.ChatInputBarConfiguration.TargetTextChannel:SendAsync('I AM USING THE Triplevape')
					else
						replicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer('I AM USING THE Triplevape', 'All')
					end
				end)
			end,
			shutdown = function()
		...
		-- Assign commands to vape
		vape.Commands = commands		
